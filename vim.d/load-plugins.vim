...
if g:flag#plug_enable
    try
        ...
        call plug#end()
        let g:status#plug_loaded = 1
    catch
        let g:status#plug_loaded = 0
        if g:flag#plug_auto_install
            echom 'Warning: Vim-Plug is not installed!'
            let choice = confirm("Try to install? (curl required)", "&Yes\n&No", 2)
            if choice == 1
                silent ! curl -fLo ~/.vim/autoload/plug.vim --create-dirs https://raw.githubusercontent.com/junegunn/vim-plug/master/plug.vim;
                echom 'Installation finished. Please run :PlugUpdate and restart vim.'
                let g:status#plug_auto_installed = 1
